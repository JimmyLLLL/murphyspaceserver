...
const { users, posts, comment } = require("../config/mysqlInitConf");
...
let insertPost = function(value) {
  let _sql =
    "insert into posts set name=?,title=?,content=?,uid=?,moment=?,avator=?;";
  return query(_sql, value);
};
...
let insertComment = function(value) {
  let _sql =
    "insert into comment set name=?,nickname=?,content=?,moment=?,postid=?,avator=?";
  return query(_sql, value);
};
...
let findPostByUserPage = function(name, page) {
  let _sql = ` select * FROM posts where uid="${name}" order by id desc limit ${(page -
    1) *
    10},10 ;`;
  return query(_sql);
};
